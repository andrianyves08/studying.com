...
if(empty($_COOKIE['admin_logged_in'])){
    $id = $_COOKIE['user_id'];
    if (!file_exists('../assets/img/media/source/'.$id)) {
        mkdir('../assets/img/media/source/'.$id, 0777, true);
    }
    $upload_dir = '/assets/img/media/source/'.$id.'/';
    $current_path = '../assets/img/media/source/'.$id.'/';
    $thumbs_base_path = '../assets/img/media/thumbs/'.$id.'/';
    $thumbs_upload_dir = '/assets/img/media/thumbs/'.$id.'/';
} else {
...
$config = array(
    ...
    |
    */
    'base_url' => ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] == "on") ? "https" : "http"). "://". @$_SERVER['HTTP_HOST'].'/studying_v2.0',
    /*
    |--------------------------------------------------------------------------
